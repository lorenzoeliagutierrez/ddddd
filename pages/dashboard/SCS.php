...
?>
<title>
    Enrolled SCS Students | SFAC - Bacoor
</title>
</head>
                ...
                <div class="col-12">
                    <div class="card shadow shadow-xl">
                        ...
                        <div class="card-header m-1 my-0"> 
                            <div class="row mb-0">
                                <div class="col mx-0">
                                    <h5 class="mb-0 ">List of Computer Science Enrollees</h5>
                                ...
                                <div class="col text-end">
                                        <div class="row">
                                            ...
                                            while ($displayCScourses = mysqli_fetch_array($CScourses)) {
                                                ...
                                                $countTotal = mysqli_query($db, "SELECT COUNT(sy_id) FROM tbl_schoolyears WHERE remark = 'Approved' AND course_id = '$displayCScourses[course_id]' AND sem_id = '$_SESSION[S]' AND ay_id = '$_SESSION[AC]' ") or die($db->error);
                                                $actualCountTotal = mysqli_fetch_array($countTotal);

                                                $countNew = mysqli_query($db, "SELECT COUNT(sy_id) FROM tbl_schoolyears WHERE remark = 'Approved' AND status = 'New' AND course_id = '$displayCScourses[course_id]' AND sem_id = '$_SESSION[S]' AND ay_id = '$_SESSION[AC]' ") or die($db->error);
                                                $actualCountNew = mysqli_fetch_array($countNew);

                                                $countOld = mysqli_query($db, "SELECT COUNT(sy_id) FROM tbl_schoolyears WHERE remark = 'Approved' AND status = 'Old' AND course_id = '$displayCScourses[course_id]' AND sem_id = '$_SESSION[S]' AND ay_id = '$_SESSION[AC]' ") or die($db->error);
                                                $actualCountOld = mysqli_fetch_array($countOld);

                                                echo'
                                                <div class="col">
                                                    <button class="btn btn-icon btn-3 btn-dark" value="'.$displayCScourses['course_id'].'" name="'.$displayCScourses['course_abv'].'">
                                                        <span class="btn-inner--icon"><i class="fas fa-laptop"></i></span>
                                                        <span class="btn-inner--text">'.$displayCScourses['course_abv'].'</span>
                            ...
                            <table class=" table table-hover responsive nowrap m-0" id="datatable-basic"
                                ...
                                <tbody>
                                    ...
                                    $pendStud = $db->query("SELECT *, CONCAT(S.firstname, ' ', S.middlename, ' ', S.lastname) AS fullname
                                            ...
                                            WHERE remark IN ('Approved') AND C.department_id IN ('3') AND ay_id IN ('$_SESSION[AC]') AND sem_id IN ('$_SESSION[S]') AND 
